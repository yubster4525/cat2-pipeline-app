...
Figure~\ref{fig:jenkins-stage} captures the stage view for Build \#14, showing each phase (Checkout, Prepare Metadata, Install Dependencies, Lint, Unit Tests, Build Docker Image, Push to Amazon ECR, Deploy to Amazon ECS, Post Actions) succeeded. Figure~\ref{fig:jenkins-console} provides the console log snippet that ends with the deployment success message, while Figure~\ref{fig:jenkins-tests} highlights the `npm test` summary. Email delivery proof is captured in Figure~\ref{fig:jenkins-email}.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\linewidth]{screenshots/jenkins-email-notification.png}
  \caption{Email notification sent by Jenkins Mailer after pipeline completion}
  \label{fig:jenkins-email}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.85\linewidth]{screenshots/ecr-scan-on-push.png}
  \caption{Security scan-on-push enabled for the ECR repository}
  \label{fig:ecr-scan}
\end{figure}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\linewidth]{screenshots/alb-https-listener.png}
  \caption{HTTPS (port 443) listener bound to an ACM certificate with HTTP\textrightarrow{}HTTPS redirect}
  \label{fig:alb-https}
\end{figure}
...
\begin{longtable}{p{0.3\linewidth}p{0.4\linewidth}p{0.2\linewidth}}
  ...
  Notification proof & Email (or Slack) success message & Fig.~\ref{fig:jenkins-email} \\
  ECR image screenshot & Repository with pushed images & Fig.~\ref{fig:ecr} \\
  Security scan evidence & ECR scan-on-push screenshot & Fig.~\ref{fig:ecr-scan} \\
  Running application endpoint & Browser screenshot of ALB DNS & Fig.~\ref{fig:endpoint} \\
  HTTPS listener evidence & ACM-backed HTTPS listener screenshot & Fig.~\ref{fig:alb-https} \\
  ...
  IaC snapshot & Task definition template listing & Listing~\ref{lst:task-def} \\
  \bottomrule
\end{longtable}

\section{Infrastructure as Code Snapshot}
The ECS task definition template checked into \texttt{infra/} keeps deployments reproducible and ready for Terraform/CloudFormation ingestion. Listing~\ref{lst:task-def} shows the JSON source used by \texttt{scripts/deploy\_to\_ecs.sh}.

\lstinputlisting[caption={Task definition template in \texttt{infra/task-definition-template.json}},label={lst:task-def}]{infra/task-definition-template.json}
